...
//MARK: - Others
func makeTextField(withPlaceholderText placeHolderText: String) -> PaddedTextField {
    let textField = PaddedTextField()
    textField.translatesAutoresizingMaskIntoConstraints = false
    textField.placeholder = placeHolderText
    textField.borderStyle = .none
    textField.layer.cornerRadius = 8.0 // You can adjust the corner radius as needed
...
func makeLinkText(withText text: String, size: CGFloat, target: Any?, action: Selector?) -> UILabel {
    let label = makeLabel(withText: text, size: size, color: .systemBlue)

    // Enable user interaction to make the label tappable
    label.isUserInteractionEnabled = true

    // Create and add a UITapGestureRecognizer to the label
